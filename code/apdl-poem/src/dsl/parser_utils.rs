...
pub fn parse_scope_desc(scope_str: &str) -> Result<ScopeDesc, String> {
    ...
    if let Some(stripped) = scope_str.strip_prefix("layer(") {
        if let Some(layer_name) = stripped.strip_suffix(')') {
    ...
    } else if let Some(stripped) = scope_str.strip_prefix("global(") {
        if let Some(scope_name) = stripped.strip_suffix(')') {
...
pub fn parse_constraint(constraint_str: &str) -> Result<Constraint, String> {
    ...
    if let Some(stripped) = constraint_str.strip_prefix("fixed(") {
        if let Some(value_str) = stripped.strip_suffix(')') {
            // 尝试解析十进制或十六进制值
            let value = if let Some(hex_str) = value_str.strip_prefix("0x") {
                u64::from_str_radix(hex_str, 16)
                    .map_err(|_| format!("Invalid hex value: {value_str}"))?
            } else if let Some(hex_str) = value_str.strip_prefix("0X") {
                u64::from_str_radix(hex_str, 16)
                    .map_err(|_| format!("Invalid hex value: {value_str}"))?
            } else {
                value_str
            ...
            Err(format!("Invalid fixed constraint format: {constraint_str}"))
        }
    } else if let Some(stripped) = constraint_str.strip_prefix("range(") {
        if let Some(range_str) = stripped.strip_suffix(')') {
            if let Some(double_dot_eq) = range_str.find("..=") {
                let start_str = &range_str[..double_dot_eq];
                let end_str = &range_str[double_dot_eq + 3..];

                // 解析起始值
                let start = if let Some(hex_str) = start_str.strip_prefix("0x") {
                    u64::from_str_radix(hex_str, 16)
                        .map_err(|_| format!("Invalid hex start value: {start_str}"))?
                } else if let Some(hex_str) = start_str.strip_prefix("0X") {
                    u64::from_str_radix(hex_str, 16)
                        .map_err(|_| format!("Invalid hex start value: {start_str}"))?
                } else {
                    start_str
                ...
                // 解析结束值
                let end = if let Some(hex_str) = end_str.strip_prefix("0x") {
                    u64::from_str_radix(hex_str, 16)
                        .map_err(|_| format!("Invalid hex end value: {end_str}"))?
                } else if let Some(hex_str) = end_str.strip_prefix("0X") {
                    u64::from_str_radix(hex_str, 16)
                        .map_err(|_| format!("Invalid hex end value: {end_str}"))?
                } else {
                    end_str
            ...
            Err(format!("Invalid range constraint format: {constraint_str}"))
        }
    } else if let Some(stripped) = constraint_str.strip_prefix("enum(") {
        if let Some(enum_str) = stripped.strip_suffix(')') {
