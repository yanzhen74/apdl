...
- **SequenceValidator**: 序列号校验器
  - 基于序列号连续性检测帧丢失
  - 支持CCSDS 14位序列号（模数0x4000），已升级为u32类型支持更大序列号范围
  - 支持序列号回绕检测和重复帧检测
  - 多通道独立校验
...
**阶段3：分层拆包和集成** ✅
- **LayeredDisassembler**: 分层拆包引擎
  - 自动识别协议层级关系，递归拆包直到应用数据层
  - 支持任意层数的嵌套协议栈（如TM Frame → Space Packet → 应用数据）
  - 中间层校验和解析
  - 完整的分层拆包结果展示（DisassembleResult）
  - 每层数据记录（LayerData）
  - 分层错误追踪（ValidationError）
  - 单元测试覆盖率100%（4个测试用例全部通过）
...
**核心API接口**:
- `disassemble_layers()`: 完整分层拆包，返回所有层的详细信息
