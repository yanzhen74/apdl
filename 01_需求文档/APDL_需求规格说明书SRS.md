...
### 1.1 项目简介
APDL（APDS Protocol Definition Language）是一个面向航天领域的协议定义与仿真验证一体化平台，通过字段级语法单元和语义规则系统，实现CCSDS、CAN等航天领域协议的灵活定义、全流程软件仿真和性能分析能力.
...
##### 3.1.4.0 字段级语法单元需求
- **FR-3.1.4.0.1**：应实现字段级语法单元而非帧级协议单元
- **FR-3.1.4.0.2**：应支持DSL描述完整的航天协议帧、包以及帧包之间的嵌套关系
- **FR-3.1.4.0.3**：应根据DSL自动进行仿真组帧，无需特定的协议实现文件
...
- **FR-3.1.4.1.5**：应支持动态长度计算（根据表达式动态确定字段长度）
...
##### 3.1.7.2 语义规则DSL规范需求
- **FR-3.1.7.2.1**：应支持22种语义规则的DSL定义语法
- **FR-3.1.7.2.2**：应支持checksum_range、dependency、order、pointer等规则类型
- **FR-3.1.7.2.3**：应支持routing_dispatch、sequence_control、validation等协议特有规则
- **FR-3.1.7.2.4**：应支持multiplexing、priority_processing、state_machine等通用规则
- **FR-3.1.7.2.5**：应支持规则参数化和描述信息
...
- **FR-3.1.8.2.5**：应支持语义规则的应用和处理
- **FR-3.1.8.2.6**：应支持字段级语法单元的组装和解析
...
### 3.1.12 语义规则系统需求
- **FR-3.1.12.1**：应支持22种语义规则类型，包括校验范围、依赖关系、顺序控制等
- **FR-3.1.12.2**：应通过DSL定义语义规则，实现零代码协议处理
- **FR-3.1.12.3**：应支持语义规则的分类管理（数据流向控制类、数据完整性保障类、数据流状态管理类、数据结构映射类、业务逻辑处理类）

### 3.1.13 数据区子包恢复需求
- **FR-3.1.13.1**：系统应支持独立的字段映射机制（连接器模式）
  - **FR-3.1.13.1.1**：应支持定义独立的映射规则，将下层包的特定字段值映射到上层包的分路字段（如VCID、APID、子系统标志、数据类型等）
  - **FR-3.1.13.1.2**：应支持通过映射规则动态确定上层包的虚拟通道或应用进程标识
  - **FR-3.1.13.1.3**：应支持多种下层包格式的字段映射规则定义
  - **FR-3.1.13.1.4**：应支持DSL语法定义field_mapping规则，格式为：`rule: field_mapping(source_package: "源包名"; target_package: "目标包名"; mappings: [{source_field: "源字段", target_field: "目标字段", mapping_logic: "映射逻辑", default_value: "默认值"}]; desc: "描述")`
- **FR-3.1.13.2**：系统应支持基于导头指针的子包处理机制
  - **FR-3.1.13.2.1**：应支持导头指针解析功能，定位数据区中的子包起始位置
  - **FR-3.1.13.2.2**：应支持将具有完整帧定义的子包注入复接包的数据区
  - **FR-3.1.13.2.3**：应实现子包边界的识别与重建，确保子包数据完整性
- **FR-3.1.13.3**：系统应区分连接器、多路复用和路由分发三种不同的语义规则
  - **FR-3.1.13.3.1**：连接器（Connector）用于跨包字段映射，解决不同协议层间的数据传递问题
  - **FR-3.1.13.3.2**：多路复用（Multiplexing）用于包内字段选择，根据条件选择不同的处理路径
  - **FR-3.1.13.3.3**：路由分发（Routing Dispatch）用于包内数据分发，决定数据包的处理路径
  - **FR-3.1.13.3.4**：三种机制作用范围不同：连接器是跨包映射，后两者是包内处理

### 3.1.14 连接器、多路复用和路由分发机制详细说明

#### 3.1.14.1 连接器（Connector）机制
连接器是一种独立的跨包字段映射机制，用于解决不同协议层之间的数据传递问题。

- **作用范围**：跨包映射，用于不同协议包之间的字段值传递
- **主要功能**：将一个包的字段值映射到另一个包的字段
- **典型应用**：将下层协议包的标识符映射到上层协议包的分路字段（如VCID、APID等）
- **DSL语法示例**：
  ```dsl
  rule: field_mapping(
...
- **作用范围**：包内选择，用于同一协议包内部的条件判断
- **主要功能**：根据条件表达式的值选择不同的处理分支
- **典型应用**：根据协议类型字段选择不同的负载格式
- **DSL语法示例**：
  ```dsl
  rule: multiplexing(
    field: "type_field";
    condition: "type == 0x01";
    route_to: "payload_a";
    desc: "Route to payload A when type is 0x01"
  )
  ```

#### 3.1.14.3 路由分发（Routing Dispatch）机制
路由分发是包内的数据分发机制，用于决定数据包的转发路径或处理方式。

- **作用范围**：包内分发，用于同一协议包内部的数据流向控制
- **主要功能**：根据字段值决定数据包的处理路径
- **典型应用**：根据目的地址决定包的转发端口
- **DSL语法示例**：
  ```dsl
  rule: routing_dispatch(
    field: "dest_addr";
    algorithm: "hash_dest_to_port";
    desc: "Hash destination address to output port"
  )
  ```

#### 3.1.14.4 三种机制的关系与区别

| 特征 | 连接器（Connector） | 多路复用（Multiplexing） | 路由分发（Routing Dispatch） |
|------|-------------------|------------------------|----------------------------|
| 作用范围 | 跨包映射 | 包内选择 | 包内分发 |
| 时机 | 解析时建立映射 | 解析时条件判断 | 解析时路由决策 |
| 目的 | 字段值传递 | 路径选择 | 转发决策 |
| 依赖关系 | 独立于父子包结构 | 依赖包内字段 | 依赖包内字段 |

这种设计使得APDL既能够处理协议层间的复杂映射关系，又能够在单个协议层内进行灵活的数据路由和选择。
